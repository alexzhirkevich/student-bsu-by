...
class HostelRepository(
    ...
    override suspend fun getFromWeb() : HostelState{
        ...
        val jsoup = Ksoup.parse(profileApi.hostel().html())
        ...
        kotlin.runCatching {
            ...
            when (value) {
                is HostelState.Provided -> {
                    preferences.putString(PREF_HOSTEL_ADDRESS_ + username, value.address)
                }
                is HostelState.NotProvided -> with(hostelDao) {
                    kotlin.runCatching {
                        preferences.remove(PREF_HOSTEL_ADDRESS_ + username)
                    }
                    clear()
